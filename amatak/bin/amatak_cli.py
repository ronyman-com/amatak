...
class Runtime:
    def __init__(self, debug):
        ...
        self.db_connections = {}  # Track active connections
        
        # Initialize database methods
        self.db_connect = self._db_connect
        self.db_execute = self._db_execute
        self.db_disconnect = self._db_disconnect

    def _db_connect(self, db_type: str, **params):
        """Connect to database and return driver instance"""
        try:
            from amatak.database import connect
            driver = connect(db_type, **params)
            if not driver:
                raise AmatakError("Failed to initialize database driver")
            
            conn_id = id(driver)
            self.db_connections[conn_id] = driver
            return driver
        except Exception as e:
            raise AmatakError(f"Database connection failed: {str(e)}")

    def _db_execute(self, driver, query: str, params=None):
        """Execute query on connected database"""
        conn_id = id(driver)
        if conn_id not in self.db_connections:
            raise AmatakError("Database not connected")
        return driver.execute(query, params or [])

    def _db_disconnect(self, driver):
        """Close database connection"""
        conn_id = id(driver)
        if conn_id in self.db_connections:
            try:
                driver.disconnect()
                del self.db_connections[conn_id]
                return True
            except Exception as e:
                raise AmatakError(f"Disconnection failed: {str(e)}")
        return False
...
class AmatakCLI:
    def __init__(self, debug: bool = False):
        self.debug = debug
        self.runtime = Runtime(self.debug)
    ...
    def setup_database_commands(self):
        ...
        if not hasattr(self.runtime, 'db_connect'):
            raise RuntimeError("Database support not initialized")
